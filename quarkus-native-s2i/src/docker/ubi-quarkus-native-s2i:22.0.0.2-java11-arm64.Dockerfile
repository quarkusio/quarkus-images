...
# ----------------
# Module graalvm 22.0.0.2-java11-arm64
...
# Artifact graalvm-java11-linux-aarch64-22.0.0.2.tar.gz downloaded from https://github.com/graalvm/graalvm-ce-builds/releases/download/vm-22.0.0.2/graalvm-ce-java11-linux-aarch64-22.0.0.2.tar.gz
COPY /artifacts/graalvm-java11-linux-aarch64-22.0.0.2.tar.gz /tmp/graalvm-java11-linux-aarch64-22.0.0.2.tar.gz
RUN tar xzf /tmp/graalvm-java11-linux-aarch64-22.0.0.2.tar.gz -C /opt \
  && mv /opt/graalvm-ce-*-22.0.0.2* /opt/graalvm \
  && /opt/graalvm/bin/gu --auto-yes install native-image \
  && rm -Rf /tmp/graalvm-java11-linux-aarch64-22.0.0.2.tar.gz
# ----------------
...
COPY /artifacts/apache-maven-3.8.4.tar.gz /tmp/apache-maven-3.8.4.tar.gz
# Artifact maven-settings.xml downloaded from jar:file:/Users/clement/.m2/repository/me/escoffier/jdock/jdock/1.0-SNAPSHOT/jdock-1.0-SNAPSHOT.jar!/maven/settings.xml
COPY /artifacts/maven-settings.xml /tmp/maven-settings.xml
# Artifact configure-maven.sh downloaded from jar:file:/Users/clement/.m2/repository/me/escoffier/jdock/jdock/1.0-SNAPSHOT/jdock-1.0-SNAPSHOT.jar!/maven/configure-maven.sh
COPY /artifacts/configure-maven.sh /tmp/configure-maven.sh
...
COPY /artifacts/gradle.zip /tmp/gradle.zip
